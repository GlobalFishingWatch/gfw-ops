...
class Compactor:
    ...
    def _compact(self, date: datetime.date) -> None:
        ...
        if staging_blobs:
            logger.warning(f"Cleaning up {len(staging_blobs)} leftover staging file(s) for {date}")
            self._delete_blobs(staging_blobs)
        ...
        logger.info(f"DuckDB done in {time.monotonic() - t0:.1f}s — {len(staged_blobs)} file(s) staged")
...
def run(
    ...
    **kwargs: Any,
    ...
    logger.info(f"Compacting {gcs_path} for [{start_date}, {end_date}) ({len(dates)} days)")

    if dry_run:
        for date in dates:
            p = partition_prefix
            logger.info(f"[dry-run] {gcs_path}/{p}source={event_source}/{p}date={date}")
        return
