...
def test_duckdb_conn_configures_connection():
    mock_conn = MagicMock()
    with patch("gfw.ops.pipelines.compact_parquet.main.duckdb.connect", return_value=mock_conn):
        with patch("gfw.ops.pipelines.compact_parquet.main.gcsfs.GCSFileSystem") as mock_fs:
            conn = _duckdb_conn(memory_limit="4GB", threads=2)

    assert conn is mock_conn
    mock_conn.execute.assert_any_call("SET memory_limit='4GB'")
    mock_conn.execute.assert_any_call("SET threads=2")
    mock_conn.execute.assert_any_call("SET preserve_insertion_order=false")
